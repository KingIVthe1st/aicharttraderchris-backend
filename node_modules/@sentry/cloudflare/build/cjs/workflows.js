 ...
 */
async function deterministicTraceIdFromInstanceId(instanceId) {
...
async function propagationContextFromInstanceId(instanceId) {
...
class WrappedWorkflowStep  {
   constructor(
     ...
     _options,
     _step,
  ) {this._instanceId = _instanceId;this._ctx = _ctx;this._options = _options;this._step = _step;}
   ...
   async do(
    ...
    const scopeForStep = core.getCurrentScope();

    const userCallback = (maybeCallback || configOrCallback) ;
    const config = typeof configOrCallback === 'function' ? undefined : configOrCallback;

    const instrumentedCallback = async () => {
      return core.startSpan(
        {
          ...
          attributes: {
            ...
            [core.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: 'auto.faas.cloudflare.workflow',
        ...
        async span => {
          try {
            ...
            core.captureException(error, { mechanism: { handled: true, type: 'auto.faas.cloudflare.workflow' } });
            throw error;
          } finally {
            this._ctx.waitUntil(core.flush(2000));
          }
        },
      );
    };
 ...
 */
function instrumentWorkflowWithSentry

(optionsCallback, WorkFlowClass) {
          ...
          if (prop === 'run') {
            return async function (event, step) {
              ...
              return core.withIsolationScope(async isolationScope => {
                const client = sdk.init({ ...options, enableDedupe: false });
...
exports.deterministicTraceIdFromInstanceId = deterministicTraceIdFromInstanceId;
exports.instrumentWorkflowWithSentry = instrumentWorkflowWithSentry;
